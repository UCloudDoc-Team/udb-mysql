...
综合考虑UDB服务的性能和稳定性,默认的最大连接数为2000. 如果用户有需求,可在控制台参数管理页面中自行调整。不同版本提供不同的最大值，具体参考参数可选最大值。

## 普通实例如何升级到高可用实例？

控制台上对普通版实例自主操作升级到高可用。

目前支持升级的类型：mysql-5.5、mysql-5.6、mysql-5.7、mysql-8.0、percona-5.5、percona-5.6、percona-5.7
